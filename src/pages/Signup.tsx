...
const Signup = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!fullName || !email || !password || !workspaceName) return;
    setLoading(true);
    ...
    // Create workspace after signup — the auth listener will set the user
    // We need a small delay for the session to be established
    setTimeout(async () => {
      try {
        await createWorkspace.mutateAsync({ name: workspaceName, agencyName: workspaceName });
        toast.success('Welcome to Approvr!', { description: 'Your workspace is ready.' });
        navigate('/dashboard');
      } catch (err: any) {
        toast.error('Workspace creation failed', { description: err.message });
      } finally {
        setLoading(false);
      }
    }, 1000);
  ...
  return (
    <div className="min-h-screen flex bg-background selection:bg-primary/20">
          ...
          <div className="max-w-md">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.5 }}>
              ...
              <p className="text-lg text-muted-foreground leading-relaxed mb-8">
                Get early access, help shape the product, and lock in founding-member pricing.
              </p>
              <div className="space-y-4">
                {["No credit card required", "Founding-member pricing locked in", "Early access to all features", "Direct line to the team"].map((feature, i) => (
      ...
      <div className="flex-1 flex items-center justify-center px-5 py-6 md:p-8 bg-card lg:bg-background relative">
        <div className="absolute top-5 left-5 md:top-8 md:left-8 lg:hidden"><Link to="/"><Logo className="h-6" /></Link></div>
        <motion.div className="w-full max-w-sm" variants={containerVariants} initial="hidden" animate="visible">
          <div className="mb-6 md:mb-10 text-center lg:text-left pt-8 md:pt-0">
            <motion.h1 variants={itemVariants} className="text-2xl md:text-3xl font-bold tracking-tighter mb-1.5 md:mb-2">Create your account</motion.h1>
            <motion.p variants={itemVariants} className="text-sm md:text-base text-muted-foreground">No credit card required. Start approving in minutes.</motion.p>
          </div>
          <motion.div variants={itemVariants} className="glass-panel p-1 bg-gradient-to-br from-border/50 via-border/10 to-transparent rounded-xl md:rounded-2xl shadow-xl">
            <form onSubmit={handleSignup} className="bg-background/80 backdrop-blur-xl p-5 md:p-8 rounded-xl md:rounded-2xl border border-border/50">
              <div className="space-y-4 md:space-y-5">
                <div className="space-y-2">
                  ...
                  <Input id="name" placeholder="Alex Rivera" value={fullName} onChange={e => setFullName(e.target.value)}
                ...
                <div className="space-y-2">
                  ...
                  <Input id="email" type="email" placeholder="alex@agency.com" value={email} onChange={e => setEmail(e.target.value)}
                    className="h-11 md:h-12 bg-muted/50 border-border focus:border-primary/50 focus:ring-primary/20 transition-all rounded-xl text-sm" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="workspace" className="text-xs font-semibold uppercase tracking-wider text-muted-foreground ml-1">Workspace Name</Label>
                  <Input id="workspace" placeholder="Rivera Design Co" value={workspaceName} onChange={e => setWorkspaceName(e.target.value)}
                    className="h-11 md:h-12 bg-muted/50 border-border focus:border-primary/50 focus:ring-primary/20 transition-all rounded-xl text-sm" />
                </div>
                <div className="space-y-2">
                ...
                <div className="pt-2">
                  <ShinyButton type="submit" className="w-full h-11 md:h-12 text-sm font-bold group rounded-lg" disabled={loading}>
          ...
          <motion.p variants={itemVariants} className="text-sm text-center text-muted-foreground mt-5 md:mt-8">
